...
## 版本紀錄
- **V7（目前版本）**：① 八字「五行分布圖」改成真正的 JS 動畫（`requestAnimationFrame` 逐格計算），長條寬度與右側數字會一起從 0 跑到目標值，並在跑分過程中加上光澤掃過效果，不再依賴容易被瀏覽器合併重繪、導致動畫失效的 CSS transition 技巧。② 姓名學結果新增「大名字招牌」與五格圖：姓名會用大字放大顯示在結果欄最上方，下方則有一個十字佈局的五格圖（天格／人格／地格／外格／總格），五個數理會以連線＋逐一彈出的動畫依序呈現，視覺上更清楚也更有儀式感。③ 星座與紫微斗數在結果生成時，新增一個由上而下擴散的「炫光閃現」動畫，並將等待文字換成帶有旋轉符文圖示的讀取提示，讓結果揭曉更有戲劇效果；此光暈與跑出動畫（chip 逐一淡入）也套用到其餘分頁的結構化標籤上，整站風格更一致。
