...
## 版本紀錄
- **V9（目前版本）**：星座與八字的「出生日期」欄位改成可直接輸入 8 碼數字（例如 `19781002`），輸入時會自動補上斜線分隔，取代原本必須點開日曆選擇的方式，操作更直覺；同時會檢查日期是否真實存在（例如不會誤收 2 月 30 日）。八字的「出生時間」改用「時／分」兩個下拉選單取代原生時間選擇器，跨瀏覽器、跨裝置的顯示與操作更一致。
