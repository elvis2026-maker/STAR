...
## 版本紀錄
- **V6（目前版本）**：① 修正八字面板「五行分布圖」長條偶爾不會跑動畫的問題——原本用兩層 `requestAnimationFrame` 觸發 CSS transition，在部分手機瀏覽器上會被瀏覽器合併成同一次重繪而不播放動畫，改成先強制觸發一次 reflow（讀取 `offsetWidth`）再套用目標寬度，動畫更穩定可靠。② 修正姓名學「手動輸入筆畫沒有用」的重大 bug——原本按下「計算五格並生成解讀」時，程式碼會無條件重新產生筆畫輸入欄位，把使用者剛剛手動填寫的筆畫值直接洗掉再讀取（永遠讀到空值或字典預設值），現在改成只有姓名字元真的改變時才重建欄位，手動輸入的筆畫會被正確保留並拿去計算。③ 筆畫字典新增一批常見名字用字（珍、維、冠、儀、雯…等），減少「未收錄請手動輸入」的情況。
